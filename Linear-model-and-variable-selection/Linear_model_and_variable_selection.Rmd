...
I observe which variable to select by keeping only ones for which p-value is < bonferroni_adjusted_alpha:
```{r}
summary(model)
```
...
Here we again select only variable *LOGSALE* with an adjusted p-value < alpha. The model will the the same as the one obtained with Bonferroni correction.
